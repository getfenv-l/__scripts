...
local Library = loadstring(game:HttpGet("https://raw.githubusercontent.com/alseyecritical/tb/refs/heads/main/tbing"))()
...
function ball_velocity.create_gui()
    ...
    main_frame.Size = UDim2.new(0, 230, 0, 120)
    main_frame.Position = UDim2.new(0, 20, 0, 150)
    main_frame.BackgroundColor3 = ball_velocity.__config.colors.background
    main_frame.BorderSizePixel = 0
    main_frame.Parent = gui
    
    ball_velocity.create_corner(12).Parent = main_frame
    ball_velocity.create_stroke(1, ball_velocity.__config.colors.border).Parent = main_frame

    local white_stroke = Instance.new("UIStroke")
    white_stroke.Thickness = 1
    white_stroke.Color = Color3.fromRGB(255, 255, 255)
    white_stroke.Transparency = 0.85
    white_stroke.Parent = main_frame
    ...
    local header_bottom_fix = Instance.new("Frame")
    header_bottom_fix.Size = UDim2.new(1, 0, 0, 12)
    header_bottom_fix.Position = UDim2.new(0, 0, 1, -12)
    header_bottom_fix.BackgroundColor3 = ball_velocity.__config.colors.header
    header_bottom_fix.BorderSizePixel = 0
    header_bottom_fix.Parent = header
    ...
    current_value.Size = UDim2.new(1, 0, 0, 22)
    current_value.Position = UDim2.new(0, 0, 0, 16)
    ...
    peak_label.Size = UDim2.new(1, 0, 0, 16)
    peak_label.Position = UDim2.new(0, 0, 0, 44)
    peak_label.BackgroundTransparency = 1
    peak_label.Text = "Peak Velocity"
    peak_label.TextColor3 = ball_velocity.__config.colors.text_secondary
    peak_label.TextSize = 11
    ...
    UserInputService.InputChanged:Connect(function(input)
        if ball_velocity.__state.is_dragging and input.UserInputType == Enum.UserInputType.MouseMovement then
            local delta = input.Position - drag_start
            main_frame.Position = UDim2.new(
...
function ball_velocity.start()
    if ball_velocity.__state.active then
        return
    end
    ...
    System.__properties.__connections.ball_velocity = RunService.Heartbeat:Connect(function()
        ...
        ball_velocity.__state.ball_data[ball] = ball_velocity.__state.ball_data[ball] or 0
        ...
        peak_value.Text = string.format("%.1f", ball_velocity.__state.ball_data[ball])
    end)
end

function ball_velocity.stop()
    if not ball_velocity.__state.active then
        return
    end
